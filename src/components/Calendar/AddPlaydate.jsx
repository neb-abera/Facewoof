...
const AddPlaydate = ({
  ...
  const [_packChoiceName, setPackChoiceName] = useState();
  ...
  const handleSubmit = async () => {
    ...
    if (!playStartTime || !playEndTime)
      return setError("Pick a start and an end time.");
  ...
  return (
    <div className="playdate-form">
      ...
      >
        ...
        {packs.map((packObj) => (
          <option key={JSON.stringify(packObj)} value={JSON.stringify(packObj)}>
            {packObj.name}
          </option>
        ))}
      </select>

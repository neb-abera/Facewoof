import { avatarUrl } from "../../../images";

interface PostTileProps {
  img?: string | null;
  content?: string | null;
  postedOn?: string | number | Date;
  parentGroup?: string;
}

/*
 * One post in the pack feed.
 *
 * It used to be a row split 25/85 — which overflows on its own — with the
 * photo, the timestamp and the pack name stacked in a narrow left column and
 * the message stranded in a card far to the right. The message is the point of
 * the post, so it leads; the timestamp and pack read as a caption underneath.
 */
const PostTile = ({ img, content, postedOn, parentGroup }: PostTileProps) => {
  const posted = postedOn === undefined ? null : new Date(postedOn);
  const when =
    posted && !Number.isNaN(posted.getTime()) ? posted.toLocaleString() : null;

  return (
    <article className="post-tile">
      {img ? (
        <img
          className="post-tile__photo"
          src={avatarUrl(img, 72)}
          width={72}
          height={72}
          loading="lazy"
          decoding="async"
          alt=""
        />
      ) : null}
      <div className="post-tile__body">
        <p className="post-tile__content">{content}</p>
        <p className="post-tile__meta">
          {parentGroup ? (
            <span className="post-tile__pack">{parentGroup}</span>
          ) : null}
          {when ? <span>{when}</span> : null}
        </p>
      </div>
    </article>
  );
};

export default PostTile;

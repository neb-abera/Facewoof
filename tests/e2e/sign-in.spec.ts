...
/* What /api/auth/providers answers. */
interface Providers {
  configured: boolean;
...
test.describe("with a provider configured", () => {
  ...
  test("signing in lands the visitor in the app with a working session", async ({
    ...
    // The callback lands on /discover, and a brand-new account is then sent
    // on to /welcome to set up (the cold sign-in test below pins that). This
    // test is about the session, so either page is arrival; waiting for
    // /discover alone raced the client-side redirect and flaked.
    await page.waitForURL(/\/(discover|welcome)$/, { timeout: 30_000 });

 ...
 */
async function contrastRatio(locator: Locator): Promise<number | null> {
  return locator.evaluate((el) => {
    ...
    const parse = (str: string): Rgb | null => {
...
test("nothing covers the navigation links", async ({ page }) => {
  ...
  const covered = await page.evaluate((): string | null => {
...
test("the add-playdate form opens centred, over a dark scrim", async ({
  ...
  expect(scrim.length, "the overlay has a colour").toBeGreaterThanOrEqual(3);
  expect(
    scrim.slice(0, 3).reduce((a, b) => a + b, 0) / 3,
    "the scrim is light",
  ).toBeLessThan(90);
});
...
test("the navbar does not overlap itself on a phone", async ({ page }) => {
  ...
  const overlap = await page.evaluate((): string | null => {
    const box = (el: Element) => el.getBoundingClientRect();
    const visible = (el: HTMLElement) => el.offsetParent !== null;
    const label = (el: Element) =>
      (el.textContent ?? "").trim().slice(0, 20) || "icon";

    const controls = [
      ...document.querySelectorAll<HTMLElement>(".navbar a, .navbar button"),
    ].filter(visible);
    for (const [i, first] of controls.entries()) {
      for (const second of controls.slice(i + 1)) {
        ...
        if (!apart && !stacked) {
          return `${label(first)} overlaps ${label(second)}`;
        }
      }
    }
    return null;
...
test("a pack feed is only readable and writable by its members", async ({
  ...
  const packs = (await page.request
    .get("/api/getpacks")
    .then((r) => r.json())) as {
    pack_id: number;
    name: string;

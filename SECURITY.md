...
800-63B. Least privilege is NIST SP 800-53 Rev. 5 AC-6. The threat model is `docs/THREAT-MODEL.md`.
...
| SP 800-53 AC-6, AC-6(1), AC-6(2) | Least privilege. Security functions and privileged accounts kept apart from ordinary use | Met | The serving identity `facewoof-mi` holds rows only: no DDL, no `TEMPORARY`, no ownership, a read-only migration ledger (`server/db/roles/runtime.sql`). Migrations run as `facewoof-migrator`, a separate managed identity, in the deploy workflow's `migrate` job, and the server does not migrate at start-up outside development. `scripts/check-db-roles.sh` has the runtime role refused 9 statements on every pull request. `docs/DEPLOY.md`, "Database roles", has the production setup and its rollback |
| SP 800-53 CM-5 | Access restrictions for change | Met | Only `main` deploys. The `production` environment's branch policy names `main` alone, and the deploy workflow refuses to run without it (`scripts/check-environment-policy.sh`, `docs/DEPLOY.md` section 5). Branch protection on `main` requires the checks to pass |

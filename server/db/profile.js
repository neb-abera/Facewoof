...
const getCurrentUserPromise = (userId) =>
  db.query(`Select * from users where user_id = ${userId}`).catch((err) => err);
...
const addToPackPromise = (packId, userId) => {
  return db
    .query('INSERT INTO pack_users(pack_id, user_id) VALUES($1, $2)', [packId, userId])
    .catch((err) => err);
};
...
const createPackPromise = (packName) => {
  return db
    .query('INSERT INTO packs(name) VALUES($1) RETURNING pack_id', [packName])
    .catch((err) => err);
};
...
const getProfilePhotoPromise = (userId) => {
  return db.query(`select url from profile_photos WHERE user_id = ${userId}`).catch((err) => err);
};

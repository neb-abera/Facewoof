...
`scripts/check-newest-majors.sh` fails when `.github/dependabot.yml` holds
back a Node or `@types/node` major, and when the node image is 45 days behind
the newest Node major, LTS or not.
